<?php
// Configurar parâmetros seguros para o cookie de sessão
session_set_cookie_params([
    'lifetime' => 21600,
    'path' => '/',
    'domain' => $_SERVER['HTTP_HOST'],
    'secure' => true,        // Assegura que os cookies são enviados apenas via HTTPS
...
// Inicialmente, considera que nenhum filtro de ano foi aplicado
$anoFiltroAplicado = false;
...
if (isset($_GET['numero_os']) && $_GET['numero_os'] !== '') {
    $whereClauses[] = '(o.N_os LIKE :numero_os OR o.Nome_os LIKE :nome_os)';
    $params[':numero_os'] = '%' . $_GET['numero_os'] . '%';
    $params[':nome_os'] = '%' . $_GET['numero_os'] . '%'; // A mesma entrada é usada para buscar pelo nome
}

if (isset($_GET['year']) && $_GET['year'] != '' && $_GET['year'] != 'all') {
    $whereClauses[] = 'YEAR(o.Dt_inicial) = :year';
    $params[':year'] = $_GET['year'];
    // Verifica se o ano filtrado é diferente do ano atual
    if ($_GET['year'] != $anoAtual) {
        $anoFiltroAplicado = true;
    }
...
// Verifica se algum filtro foi aplicado (excluindo o filtro de ano, se for o ano atual)
$filtrosAplicados =
    (isset($_GET['responsavel']) && $_GET['responsavel'] != '') ||
    (isset($_GET['status_contratada']) && $_GET['status_contratada'] != '') ||
    (isset($_GET['os_paga']) && $_GET['os_paga'] != '') ||
    (isset($_GET['prioridade']) && $_GET['prioridade'] != '') ||
    (isset($_GET['numero_os']) && $_GET['numero_os'] != '') ||
    $anoFiltroAplicado;

// Se nenhum filtro foi aplicado, então adicione a cláusula para excluir OS "Em Produção" e pagas
if (!$filtrosAplicados) {
    $whereClauses[] = "NOT (o.Status_contratada = 'Em Produção' AND o.Os_paga = '1')";
}

// O restante do código para construir a consulta permanece o mesmo
$whereSql = $whereClauses ? ' WHERE ' . implode(' AND ', $whereClauses) : '';

// **Atualização da Query para ordenar pelas OS destacadas**
$query = "
    ...
    $whereSql
    ORDER BY o.Highlighted DESC, FIELD(o.Prioridade, 'Alta', 'Média', 'Baixa'), o.N_os ASC
";
...
$usuarios = $pdo->query("SELECT Id, Nome FROM usuarios WHERE PerfilAcesso IN (1, 2, 4, 9) ORDER BY Nome ASC")->fetchAll(PDO::FETCH_ASSOC);
...
$statusColorsContratada = [
    ...
    'Criando PF' => 'bg-info', // Azul claro
    'Paralisado' => 'bg-danger', // Vermelho
    'Aguardando Inova' => 'bg-sky',
...
// Consulta para calcular a soma total de PF e Valor
$somaTotalQuery = "SELECT SUM(o.Apf) AS SomaApf, SUM(o.Valor) AS SomaValor FROM os o {$whereSql}";
...
$totalOsStmt = $pdo->prepare("SELECT COUNT(*) FROM os o {$whereSql}");
...
/* Mantenha o estilo de hover para linhas destacadas */
...
.bg-sky {
    background-color: #e98c77c9; /* Sky Blue color */
}
...
.os-details p {
    white-space: pre-wrap; /* Respeita as quebras de linha no conteúdo */
    margin-bottom: 0.3em; /* Espaçamento pequeno entre as linhas */
    line-height: 1.3; /* Ajusta a altura das linhas para um valor mais compacto */
...
#viewDescricao {
    ...
    line-height: 10px; /* Ajusta a altura das linhas para um valor normal */
      ...
      <div class="page-wrapper">
        ...
        <div class="row page-titles">
          <div class="col-md-5 col-12 align-self-center">
            <h3 class="text-themecolor mb-0"><i class="mdi mdi-slack"></i> Ordens de Serviço</h3>            
          </div>          
        </div>
    ...
    <div class="card-body">
      <div class="d-flex flex-row">
        ...
        <div>
          ...
          <p class="card-text text-muted"><?php echo $somaTotalResult['SomaApf']; ?></p>
        </div>
      </div>
    </div>
  </div>
...
<div class="col-lg-3 col-md-6">
  <div class="card">
    <div class="card-body">
      <div class="d-flex flex-row">
        <div class="round round-lg text-white rounded-circle bg-success">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-wallet">
            <path d="M21 18a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v12z"></path>
            <polyline points="17 6 23 6 23 8"></polyline>
            <line x1="23" y1="11" x2="23" y2="11"></line>
          </svg>
        </div>
        <div>
          <h3 class="card-title">Valor Disponível</h3>
          <p class="card-text text-muted"><?php echo 'R$ ' . number_format($valorTotalDisponivel, 2, ',', '.'); ?></p>
        </div>
      </div>
    </div>
  </div>
...
<!-- Botão para abrir o modal de adicionar nova OS -->
        <button class="btn waves-effect waves-light btn-info" onclick="openModal('addOsModal')"><i class="ti-plus text"></i> Cadastrar O.S</button>

        <br><br>

        <!-- Filtro por Ano -->
        <div class="filter-container d-flex flex-wrap align-items-center justify-content-between">
    <form id="filterOsForm" action="os.php" method="get" class="d-flex flex-wrap align-items-center w-100">
...
<select name="responsavel">
    ...
    <?php endforeach; ?>
        ...
        <input type="text" name="numero_os" placeholder="Digite o número ou nome da OS" value="<?php echo isset($_GET['numero_os']) ? $_GET['numero_os'] : ''; ?>" />
...
<br>
    ...
    <table class="table table-striped table mb-0 content-table" data-tablesaw-mode="columntoggle" id="tablesaw-6204">
    ...
    // Determine se a linha deve ter o estilo de finalizada
            ...
            <tr role="row" class="odd <?php echo $finalizadaClass; ?> <?php if ($os['Highlighted']) echo 'highlighted'; ?>">
                <td style="text-align: center; vertical-align: middle;">
                    <input type="checkbox" class="priority-check" data-os-id="<?php echo $os['Id']; ?>" <?php if ($os['Highlighted']) echo 'checked'; ?> onclick="togglePriority(this, <?php echo $os['Id']; ?>)">
                </td>
         ...
         <td style="text-align: center; font-weight: 500; vertical-align: middle;"><?php echo htmlspecialchars($os['Apf']); ?></td>
         <td style="text-align: center; font-weight: 500; vertical-align: middle;"><?php echo 'R$ ' . htmlspecialchars(number_format($os['Valor'], 2, ',', '.')); ?></td>        
        <td style="text-align: center; vertical-align: middle;">
    <span class="badge <?php echo $prioridadesCores[$os['Prioridade']] ?? 'bg-default'; ?>">
        <?php echo htmlspecialchars($os['Prioridade']); ?>
    </span>
</td>        
<td style="text-align: center; vertical-align: middle;">
    <span class="badge <?php echo $statusColorsContratada[$os['Status_contratada']] ?? 'bg-default'; ?>">
        <?php echo htmlspecialchars($os['Status_contratada']); ?>
    </span>
</td>        
        <td style="text-align: center; font-weight: 500; vertical-align: middle;"><?php echo htmlspecialchars($os['NomeResponsavel']); ?></td> <!-- Exibindo o nome do responsável -->

       <td style="text-align: center; vertical-align: middle; position: relative;">
  <label class="switch">
    <input type="checkbox" class="os-paga-switch" data-os-id="<?php echo $os['Id']; ?>" <?php if ($os['Os_paga']) echo 'checked'; ?> onmouseover="showTooltip(this)" onmouseout="hideTooltip(this)">
    <span class="slider"></span>
    <span class="tooltip-text" style="visibility: hidden; opacity: 0; position: absolute; bottom: 125%; left: 50%; transform: translateX(-50%); background: #333; color: #fff; padding: 5px; border-radius: 4px; font-size: 12px; white-space: nowrap; z-index: 1000; transition: opacity 0.3s;">
      Tooltip
    </span>
  </label>
</td>
...
<script>
  function showTooltip(checkbox) {
    var tooltip = checkbox.nextElementSibling.nextElementSibling;
    tooltip.style.visibility = 'visible';
    tooltip.style.opacity = '1';
    tooltip.textContent = checkbox.checked ? 'OS paga' : 'OS não paga';
  }

  function hideTooltip(checkbox) {
    var tooltip = checkbox.nextElementSibling.nextElementSibling;
    ...
    <div class="d-flex justify-content-center align-items-center">
        ...
        <!-- Botão Gerar OS -->
        <button onclick="window.open('gerar_pdf_os.php?osId=<?php echo $os['Id']; ?>', '_blank')" title="Gerar O.S" class="btn btn-sm btn-light-info text-white mx-1">
    ...
    <?php else: ?>
                <tr>
                    <td colspan="13" style="text-align:center;">Nenhuma OS encontrada</td>
                </tr>
            <?php endif; ?>
        </tbody>
    ...
    <nav aria-label="Navegação de página exemplo">
        <ul class="pagination">
            ...
            $baseURL = 'os?' . http_build_query($parametrosFiltro) . '&pagina=';
...
<div id="addOsModal" class="modal" style="display: none;">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 90%;">
        <div class="modal-content bg-dark text-white">
            <div class="modal-header">
                <h2 class="modal-title" id="modalCadastroOsLabel"><i class="mdi mdi-slack"></i>&nbsp;Cadastrar O.S</h2>
        ...
        <form id="addOsForm" action="process_add_os.php" method="post" enctype="multipart/form-data">
            ...
            <!-- Número da OS -->
            <div class="mb-3">
                <label for="addNOs" class="form-label">N° OS</label>
                <input style="background-color: #000" type="text" class="form-control bg-secondary text-white readonly" id="addNOs" name="n_os" value="<?php echo $proximoNumeroOS; ?>" readonly>
            </div>
            
            <!-- Nome da OS -->
            <div class="mb-3">
              <label for="addNomeOs" class="form-label">Nome da OS</label>
              <input type="text" class="form-control bg-secondary text-white" id="addNomeOs" name="nome_os" required>
            </div>        
            
            
            <div class="mb-3">
    <label for="addApf" class="form-label">PF</label>
    <input type="text" class="form-control bg-secondary text-white" id="addApf" name="apf" oninput="formatInput(this); calculateValue();">
</div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control bg-secondary text-white" placeholder="R$" id="addValor" name="valor" readonly>
            </div>
    ...
    <select class="form-select" id="addStatusInova" name="status_inova" required>
        <option value="Aguardando PF">Aguardando PF</option>
        ...
        <option value="Finalizado">Finalizado</option>
            ...
            <div class="mb-3">
    <label for="addStatusContratada" class="form-label">Status egSYS</label>
    <select class="form-select" id="addStatusContratada" name="status_contratada" required>
        <option value="Não Começou">Não Começou</option>
        <option value="Paralisado">Paralisado</option>        
        <option value="Em Análise">Em Análise</option>
        <option value="Criando PF">Criando PF</option>
        <option value="Aguardando Inova">Aguardando Inova</option>  
...
<div class="mb-3">
    ...
    <select class="form-select" id="addResponsavel" name="responsavel" required>
...
<div class="mb-3">
    ...
    <select class="form-select" id="addIdContratada" name="id_contratada" required>
...
<!-- Campo de Descrição da O.S (corrigido) -->
<div class="mb-3">
    <label for="addDescricao" class="form-label">Descrição da O.S</label>
    <textarea class="form-control bg-secondary text-white" id="addDescricao" name="descricao" style="height: 150px;"></textarea>
</div>


<div class="mb-3" style="display: none;">
    <label for="addObservacao" class="form-label">Complemento</label>
...
<div id="viewOsModal" class="modal" style="display: none;">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 90%;">
        ...
        <div class="os-details">
            ...
            <div class="mb-3">
                <h3 style="font-size:18px">Nº OS</h3>
                <p style="font-size:15px" id="viewNOs"></p>
            </div>
            <div class="mb-3">
                <h3 style="font-size:18px">Nome da OS</h3>
                <p style="font-size:15px" id="viewNomeOs"></p>
            </div>
            <div class="mb-3">
                <h3 style="font-size:18px">PF</h3>
            ...
            <div class="mb-3">
                <h3 style="font-size:18px">Status egSYS</h3>
                <p style="font-size:15px" id="viewStatusContratada"></p>
            </div>
            <div class="mb-3">
            ...
            <div class="os-details">
                <h3 style="font-size:18px">Descrição da O.S</h3>
                <p id="viewDescricao"></p> <!-- Div para exibir a descrição corretamente -->
            </div>
...
<!-- Modal de Edição de OS -->
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width: 90%;">
        <div class="modal-content bg-dark text-white">
            <div class="modal-header">
                <h2 class="modal-title" id="modalCadastroOsLabel"><i class="mdi mdi-pencil"></i>&nbsp;Editar O.S </h2>
        ...
        <form id="editOsForm" action="process_edit_os.php" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <label for="editNOs" class="form-label">N° OS</label>
                <input type="text" class="form-control bg-secondary text-white" id="editNOs" name="n_os" readonly class="readonly">
            </div>

            <div class="mb-3">
                <label for="editNomeOs" class="form-label">Nome da OS</label>
                <input type="text" class="form-control bg-secondary text-white" id="editNomeOs" name="nome_os" required>
            </div>

                <div class="mb-3">
                    <label for="editApf" class="form-label">PF</label>
                    <input type="text" class="form-control bg-secondary text-white" id="editApf" name="apf" oninput="calculateValue('edit')">
                 </div>
                <div class="mb-3">
                    <label for="editValor" class="form-label">Valor</label>
                    <input type="text" class="form-control bg-secondary text-white" placeholder="R$" id="editValor" name="valor" readonly class="readonly">
                </div>
...
<div class="mb-3">
    <label for="editStatusContratada" class="form-label">Status egSYS</label>
    <select class="form-select" id="editStatusContratada" name="status_contratada" required>
        <option value="Não Começou">Não Começou</option>
        <option value="Paralisado">Paralisado</option>        
        <option value="Em Análise">Em Análise</option>
        <option value="Criando PF">Criando PF</option>
    ...
    <select class="form-select" id="editResponsavel" name="responsavel" required>
        <?php foreach ($usuarios as $usuario): ?>
    ...
    <select class="form-select" id="editIdContratada" name="id_contratada" required>
        <?php foreach ($contratadas as $contratada): ?>
        ...
        <?php endforeach; ?>
         ...
         <div class="mb-3">
                <label for="editOsPaga" class="form-label">OS Paga</label>
            ...
            <div class="mb-3">
    <label for="editDescricao" class="form-label">Descrição da O.S</label>
    <textarea class="form-control bg-secondary text-white" id="editDescricao" name="descricao" style="height: 250px"></textarea>
</div>
 

             <div class="mb-3">
            <label for="editObservacao" class="form-label">Complemento</label>
...
<script>
    ...
    // Redirecionar para 'os.php' sem parâmetros após o modal ser fechado
...
function togglePriority(checkbox, osId) {
    ...
    console.log('Checkbox clicked. osId:', osId, 'isChecked:', isChecked);
    ...
    xhr.withCredentials = true; // Certifica-se de que cookies são enviados
    xhr.send('osId=' + encodeURIComponent(osId) + '&highlighted=' + encodeURIComponent(isChecked));

    xhr.onreadystatechange = function() {
        if (xhr.readyState == XMLHttpRequest.DONE) {
            console.log('Server response:', xhr.responseText);
            if (xhr.status != 200) {
                alert('Erro ao atualizar a prioridade no servidor.');
            } else {
                // Atualiza a ordenação das linhas após a atualização no servidor
                sortHighlightedRows();
            }
        }
    };
...
function sortHighlightedRows() {
        ...
        rows.sort(function(a, b) {
            ...
            if (highlightedA && !highlightedB) {
                ...
                // Ordena por prioridade e depois por número da OS
                var priorityOrder = { 'Alta': 1, 'Média': 2, 'Baixa': 3 };

                var priorityA = priorityOrder[a.cells[6].textContent.trim()] || 4;
                var priorityB = priorityOrder[b.cells[6].textContent.trim()] || 4;
                ...
                var osNumberA = parseInt(a.cells[1].textContent.trim().split('-')[0], 10);
                var osNumberB = parseInt(b.cells[1].textContent.trim().split('-')[0], 10);
...
// Inicialização no modal de cadastro
function openModal(modalId) {
    ...
    if (modal) {
        ...
        if (modalId === 'addOsModal') {
            ...
            // Inicializa o TinyMCE para o campo de descrição
            if (tinymce.get('addDescricao')) {
                tinymce.get('addDescricao').remove();
            }
            tinymce.init({
                selector: '#addDescricao',
                plugins: 'lists advlist',
                toolbar: 'undo redo | bold italic underline | bullist numlist',
                menubar: false,
                content_style: 'body { background-color: #272b34; color: #b2b9b2; }'
            });
        }
    }
}
...
function calculateValue(context = 'add') {
    ...
    if (apfValue !== '') {
        var parsedApf = parseFloat(apfValue);
        if (!isNaN(parsedApf) && parsedApf > 0) {
            var calculatedValue = parsedApf * baseValue;
            ...
            // Se 'apf' for 0 ou inválido, define o valor para 0
            valorInput.value = 'R$ 0,00';
            valorNumericoInput.value = '0.00';
        }
    } else {
        valorInput.value = 'R$ 0,00';
        valorNumericoInput.value = '0.00';
...
document.getElementById('addOsForm').addEventListener('submit', function(event) {
    // Obtém o conteúdo do TinyMCE como texto simples
    var descricao = tinymce.get('addDescricao').getContent({ format: 'text' }).trim();
    
    if (descricao === '') {
        event.preventDefault(); // Impede a submissão do formulário
        alert('Por favor, preencha a descrição da O.S.');
        tinymce.get('addDescricao').focus(); // Foca no editor TinyMCE
    } else {
        // Continua com a submissão do formulário
        var formData = new FormData(this);

        fetch('process_add_os.php', {
            method: 'POST',
            body: formData
        })
        ...
        .then(data => {
            ...
            if (data.includes('sucesso')) {
                // Opcional: Atualize a tabela de OS aqui via AJAX ou recarregue a página
...
// Função para calcular o valor automaticamente e atualizar o campo oculto
function calculateValue(context = 'add') {
    var apfInput = document.getElementById(context + 'Apf');
    // Chama a função para substituir vírgulas por pontos e formatar o input corretamente
    formatInput(apfInput);
...
function viewOsDetails(osId) {
    ...
    .then(data => {
        ...
        // restante do código para preencher os campos do modal
    })
    .catch(error => {
...
function fetchAttachments(osId) {
    return fetch('get_os_attachments.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `os_id=${osId}`
    }).then(response => response.json());
...
function viewOsDetails(osId) {
    ...
    .then(response => response.json())
    .then(data => {
        console.log('Dados recebidos:', data); // Log para verificar todos os dados recebidos
        ...
        updateModalContent('viewValor', data.Valor ? formatarMoeda(parseFloat(data.Valor)) : '');
        updateModalContent('viewDtInicial', data.Dt_inicial ? formatarData(data.Dt_inicial) : '');
        updateModalContent('viewPrazoEntrega', data.Prazo_entrega ? formatarData(data.Prazo_entrega) : '');
        ...
        // Corrigir a descrição para preservar as quebras de linha
        updateModalContent('viewDescricao', convertLineBreaksToHtml(data.Descricao || ''));
...
<script>
document.querySelectorAll('.os-paga-switch').forEach(function(switchElem) {
    switchElem.addEventListener('change', function() {
        var osId = this.dataset.osId;
        var isPaid = this.checked ? 1 : 0;

        // Enviando requisição AJAX para atualizar o valor de 'Os_paga'
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'update_os_paga.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.send('osId=' + encodeURIComponent(osId) + '&osPaga=' + encodeURIComponent(isPaid));

        xhr.onreadystatechange = function() {
            if (xhr.readyState == XMLHttpRequest.DONE) {
                if (xhr.status != 200) {
                    alert('Erro ao atualizar o status de pagamento no servidor.');
                }
            }
        };
    });
...
<script>
async function openEditModal(osData) {
    ...
    // Verifica se o TinyMCE já está inicializado
    if (tinymce.get('editDescricao')) {
        // Atualiza o conteúdo do TinyMCE com a descrição da OS
        tinymce.get('editDescricao').setContent(osData.Descricao || '');
    } else {
        // Inicializa o TinyMCE e define o conteúdo
        initTinyMCE(osData.Descricao || '');
    }

    // Busca os anexos da OS e os exibe
    await fetchAnexos(osData.Id);

    // Abre o modal de edição
    document.getElementById('editOsModal').style.display = 'block';
}

// Função para inicializar o TinyMCE para o campo de descrição do modal de edição
function initTinyMCE(content = '') {
    // Remove o editor TinyMCE anterior, se existir
    if (tinymce.get('editDescricao')) {
        tinymce.get('editDescricao').remove();
    }

    // Inicializa o TinyMCE com o conteúdo fornecido
    tinymce.init({
        selector: '#editDescricao',
        plugins: 'lists advlist',
        toolbar: 'undo redo | bold italic underline | bullist numlist',
        menubar: false,
        setup: function (editor) {
            editor.on('init', function () {
                editor.setContent(content);  // Define o conteúdo inicial
            });
        },
        content_style: 'body { background-color: #272b34; color: #b2b9b2; }'
    });
...
// Função para buscar anexos da OS
async function fetchAnexos(osId) {
    const response = await fetch('get_os_attachments.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `os_id=${osId}`
    });
...
function deleteAnexo(anexoId, osId) {
    ...
    fetch('excluir_anexo_os.php', {
        ...
        .then(data => {
            ...
            fetchAnexos(osId);  // Recarrega os anexos após a remoção
        })
...
function updateModalContent(elementId, content) {
    var element = document.getElementById(elementId);
    if (element) {
        element.innerHTML = convertLineBreaksToHtml(content); // Utiliza innerHTML para interpretar <br> como quebra de linha
    } else {
        console.error(`Elemento '${elementId}' não encontrado.`);
    }
}
