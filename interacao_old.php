...
// Preparar a consulta SQL para filtrar e paginar corretamente
$query = "
    SELECT cards.*,
...
$stmt->bindValue(':isArchivedCondition', $isArchivedCondition, PDO::PARAM_INT);
...
// Consulta SQL para contar o número total de cards considerando o filtro
$totalCardsQuery = "SELECT COUNT(*) FROM cards WHERE is_archived = :isArchivedCondition";
$totalCardsStmt = $pdo->prepare($totalCardsQuery);
$totalCardsStmt->bindValue(':isArchivedCondition', $isArchivedCondition, PDO::PARAM_INT);
$totalCardsStmt->execute();
$totalCards = $totalCardsStmt->fetchColumn();
...
$(document).ready(function() {
    $('.searchbar input').on('input', function() {
        ...
        if (searchTerm.length > 2) {
            ...
            // Aqui você deve implementar a lógica para restaurar a visualização padrão
            // Isso pode significar uma nova chamada AJAX ou simplesmente recarregar os cards
            // que você já pode ter armazenado em alguma variável global.
            restoreDefaultView();
        }
    });
});
...
function restoreDefaultView() {
    // Suponha que você tenha uma função que carrega os cards padrão
    loadDefaultCards();
...
// Função para atualizar a variável global 'cards' com os resultados da busca
function updateCardsFromSearch(data) {
    cards = data;
}

function createCardHtml(card) {
        console.log(card); // Adicione esta linha para verificar os dados do cartão

        // Verifica o estado de arquivamento do card e define o ícone correspondente
    var archiveIconClass = card.is_archived === 0 ? 'fa fa-archive' : 'ti-menu';
        ...
        .append($('<div></div>').addClass('card-header')
            .append($('<h2></h2>').html(`<span class="badge bg-danger" style="font-size: 17px;">${card.subtaskCount || 0}</span> - ${card.title}`).css({
                'white-space': 'nowrap',
                'overflow': 'hidden',
            ...
            .append($('<div></div>').addClass('card-actions')
                ...
                .append(createActionButton(archiveIconClass, 'archive-card-btn', {'data-card-id': card.id}, card.is_archived === 0 ? 'Arquivar' : 'Desarquivar', function() { toggleArchiveState(card.id); }))
    ...
    console.log('Card clicked:', card); // Adicione esta linha para depurar
...
/// As funções de ação como viewCardDetails, editCard, toggleArchiveState, deleteCard precisam ser definidas em algum lugar do seu script
// Por exemplo:
function toggleArchiveState(cardId, isArchived) {
    // Implemente a lógica para alternar o estado de arquivamento aqui
    var newState = isArchived ? 0 : 1; // Alterna o estado
        $.ajax({
            url: 'archive_card.php',
            type: 'POST',
            data: { cardId: cardId, isArchived: newState },
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    // Encontre o card pelo ID
                    var cardElement = $('[data-card-id="' + cardId + '"]');
                    var button = cardElement.find('.archive-card-btn');
                    var icon = button.find('i');

                    // Atualize a classe do ícone e o atributo title do botão
                    icon.removeClass('fa-archive ti-menu');
                    icon.addClass(newState === 0 ? 'fa fa-archive' : 'ti-menu');
                    button.attr('title', newState === 0 ? 'Arquivar' : 'Voltar p/ pendente');

                    // Atualize o estado de arquivamento do card
                    card.is_archived = newState;
                } else {
...
// Supondo que `cards` seja carregado com todos os cartões disponíveis
var cards = [];

// Função chamada após a busca, para atualizar a variável `cards`
function updateCardsFromSearch(data) {
    cards = data; // Atualiza a variável global com os cartões buscados
}

// A função viewCardDetails agora deve funcionar, pois `cards` está definida
function viewCardDetails(cardId) {
    // Encontra o card específico usando o ID
    const card = cards.find(c => c.id === cardId);
    
    // Se o card não for encontrado, exibe um alerta e sai da função
    if (!card) {
        alert('Card não encontrado.');
        return;
    }
    
    // Atualiza o conteúdo do modal com os detalhes do card
    // Você precisará ajustar os seletores aqui para corresponder ao seu HTML
    $('#cardDetailsModal .card-title').text(card.title);
    $('#cardDetailsModal .card-responsible').text(card.responsible_users || "Não especificado");
    $('#cardDetailsModal .card-subtaskCount').text(card.subtaskCount || 0);
...
$('.searchbar input').on('input', function() {
    ...
    if (searchTerm.length > 2) {
        $.ajax({
            ...
            dataType: 'json',
            success: function(data) {
                updateCardsFromSearch(data); // Atualiza os cartões com base na busca
                $('#taskboard').empty();
                data.forEach(function(card) {
                    $('#taskboard').append(createCardHtml(card));
                });
                $('.pagination').hide(); // Esconde a paginação após a busca
            },
            error: function(xhr, status, error) {
                console.error('Erro na busca:', error);
            }
        ...
        // Aqui você pode adicionar a lógica para restaurar a visualização padrão, se necessário
    }
});


// Função para alternar o estado de arquivamento do card
function toggleArchiveState(cardId) {
    // Implemente a lógica para alternar o estado de arquivamento aqui
}
