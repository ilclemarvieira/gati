...
// Verifica se um ano foi passado via GET
if (isset($_GET['year']) && $_GET['year'] != '') {
    $anoFiltrado = $_GET['year'];
    $whereClauses[] = 'YEAR(b.Dt_criacao) = :year';
    $params[':year'] = $anoFiltrado;
...
$perfisPermitidos = [1, 2, 3, 4]; // Exemplo: somente Admin, Gestor e Inova podem acessar
verificarPermissao($perfisPermitidos);

// Adicione no início do arquivo para buscar os usuários cadastrados
$usuarios = $pdo->query("SELECT Id, Nome FROM usuarios WHERE PerfilAcesso IN (1, 2, 4)")->fetchAll(PDO::FETCH_ASSOC);
...
// Verifica se algum filtro foi aplicado (excluindo o filtro de ano)
$filtrosAplicados = 
    (isset($_GET['responsavel']) && $_GET['responsavel'] != '') || 
    (isset($_GET['status']) && $_GET['status'] != '') || 
    (isset($_GET['prioridade']) && $_GET['prioridade'] != '') || 
    (isset($_GET['encaminhado_os']) && $_GET['encaminhado_os'] != '');

// Se nenhum filtro foi aplicado, então adicione a cláusula para incluir somente os encaminhados_os = 0
if (!$filtrosAplicados) {
    $whereClauses[] = "b.Encaminhado_os = 0";
}

// Cria a parte WHERE da consulta SQL
$whereSql = !empty($whereClauses) ? ' WHERE ' . implode(' AND ', $whereClauses) : '';

// Defina os itens por página e o offset antes de usar na consulta SQL
$itensPorPagina = 12; // Definir quantos itens por página
...
$totalRegistrosStmt = $pdo->prepare("SELECT COUNT(*) FROM backlog b {$whereSql}");
...
// Substitua a consulta geral pelo total de backlogs pela consulta filtrada por ano.
$totalBacklogsStmt = $pdo->prepare("SELECT COUNT(*) FROM backlog WHERE YEAR(Dt_criacao) = :ano");
$totalBacklogsStmt->bindParam(':ano', $anoFiltrado, PDO::PARAM_INT);
$totalBacklogsStmt->execute();
$totalBacklogs = $totalBacklogsStmt->fetchColumn();

// Total de backlogs com prioridade alta excluindo "Cancelado" e os enviados para os
$totalPrioridadeAltaStmt = $pdo->prepare("SELECT COUNT(*) FROM backlog WHERE Prioridade = 'Alta' AND Status_ideia <> 'Cancelado' AND Encaminhado_os <> 1");
$totalPrioridadeAltaStmt->execute();
$totalPrioridadeAlta = $totalPrioridadeAltaStmt->fetchColumn();

// Total de backlogs com prioridade média excluindo "Cancelado" e os enviados para os
$totalPrioridadeMediaStmt = $pdo->prepare("SELECT COUNT(*) FROM backlog WHERE Prioridade = 'Média' AND Status_ideia <> 'Cancelado' AND Encaminhado_os <> 1");
$totalPrioridadeMediaStmt->execute();
$totalPrioridadeMedia = $totalPrioridadeMediaStmt->fetchColumn();

// Total de backlogs com prioridade baixa excluindo "Cancelado" e os enviados para os
$totalPrioridadeBaixaStmt = $pdo->prepare("SELECT COUNT(*) FROM backlog WHERE Prioridade = 'Baixa' AND Status_ideia <> 'Cancelado' AND Encaminhado_os <> 1");
...
foreach ($statusOptions as $status) {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM backlog WHERE Status_ideia = ?");
    $stmt->execute([$status]);
      ...
      <div class="page-wrapper">
        ...
        <div class="row page-titles">
          <div class="col-md-5 col-12 align-self-center">
            <h3 class="text-themecolor mb-0"><i class="mdi mdi-lightbulb-on"></i> Backlog</h3>            
          </div>          
        </div>
       ...
       <div class="row">
  <!-- Card Total de Backlogs -->
  <div class="col-lg-3 col-md-6">
    <div class="card" onclick="filterBacklogs('')">
      <div class="card-body">
        <div class="d-flex flex-row">
          <div class="round round-lg text-white rounded-circle bg-info">
            <i class="fa fa-list-alt"></i> <!-- Ícone atualizado para Total de Backlogs -->
          </div>
          <div>
            <h3 class="card-title">Total de Backlogs</h3>
            <p id="totalBacklogs" class="card-text text-muted"><?php echo $totalBacklogs; ?></p>
          </div>
        </div>
      </div>
    </div>
...
<!-- Botão para abrir o modal de adicionar nova OS -->
        <button class="btn waves-effect waves-light btn-info" onclick="openModal('addModal')"><i class="ti-plus text"></i> Cadastrar Backlog</button>
    ...
    <form id="filterBacklogForm" action="backlog.php" method="get" class="d-flex flex-wrap align-items-center w-100">

       <!-- Filtro por Ano -->
<select id="anoSelecionado" name="year">
    
    <?php foreach ($years as $year): ?>
        <option value="<?php echo $year; ?>" <?php if ($anoFiltrado == $year) echo 'selected'; ?>>
            <?php echo $year; ?>
        </option>
    <?php endforeach; ?>
</select>
        ...
        <select name="encaminhado_os">
            <option value="">Todos Enviados para OS</option>
...
<br>
          ...
          <div class="row">
            ...
            <thead class="thead-light" align="center">
                <tr style="background-color: #5b5f69">
                    ...
                    <th scope="col" style="white-space: nowrap;">Enviado para OS</th>
                    <th></th>
                </tr>
            </thead>
    ...
    <?php foreach ($backlogs as $backlog): ?>
        ...
        <tr class="<?php echo $backlog['Encaminhado_os'] ? 'linha-taxada' : ''; ?>">

          <td style="text-align: center; vertical-align: middle;">
              <input type="checkbox" class="priority-check" data-backlog-id="<?php echo $backlog['Id']; ?>">
          </td>


         <td style="text-align: left; font-weight: 500; text-transform: uppercase; vertical-align: middle" data-label="Projeto"><?php echo htmlspecialchars($backlog['Projeto']); ?></td>

                    <td style="text-align: center; font-weight: 500; vertical-align: middle" data-label="Data de Criação"><?php echo date('d/m/Y', strtotime($backlog['Dt_criacao'])); ?></td>

                    <td style="text-align: center; vertical-align: middle" data-label="Prioridade">
                        <?php 
                        switch ($backlog['Prioridade']) {
                            case 'Alta':
                    ...
                    <td style="text-align: center; vertical-align: middle" data-label="Status">
                        ...
                        switch ($backlog['Status_ideia']) {
                            ...
                            case 'Cancelada':
                                ...
                                break;
                            case 'Autorizada para O.S':
                                echo '<span class="badge bg-info">Autorizada para O.S</span>';
                                break;
                        }
                        ?>
                    </td>
                    ...
                    <td style="text-align: center; font-weight: 500; vertical-align: middle" data-label="Responsável"><?php echo htmlspecialchars($backlog['NomeResponsavel'] ?? 'N/A'); ?></td>

                    <td style="text-align: center; vertical-align: middle" data-label="Enviado para OS">
                        <?php if ($backlog['Encaminhado_os']): ?>
    ...
    <div class="d-flex justify-content-center align-items-center">
        <!-- Botão Ver -->
        <button onclick="viewBacklogDetails(<?php echo $backlog['Id']; ?>)" title="Ver" class="btn btn-sm btn-light-info text-white mx-1">
            <i data-feather="eye" class="feather-sm fill-white"></i>
        </button>

        <!-- Botão Editar -->
        <button onclick="loadBacklogDetails(<?php echo $backlog['Id']; ?>)" title="Editar" class="btn btn-sm btn-light-info text-white mx-1">
            <i data-feather="edit" class="feather-sm fill-white"></i>
        </button>

        <!-- Botão Gerar PDF -->
        <button onclick="window.open('gerarpdf_backlog.php?id=<?php echo $backlog['Id']; ?>', '_blank')" title="Gerar PDF" class="btn btn-sm btn-light-info text-white mx-1">
            <i data-feather="file-text" class="feather-sm fill-white"></i>
        </button>


        <!-- Botão Excluir -->
        <button onclick="deleteBacklog(<?php echo $backlog['Id']; ?>)" title="Excluir" class="btn btn-sm btn-light-danger text-white mx-1">
    ...
    <?php else: ?>
                <tr>
                    <td colspan="13" style="text-align:center;">Nenhum backlog encontrado</td>
                </tr>
            <?php endif; ?>
                </tbody>
        </table>
    ...
    <nav aria-label="Navegação de página exemplo">
        <ul class="pagination">
            ...
            $baseURL = 'backlog?' . http_build_query($parametrosFiltro) . '&pagina=';
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width: 90%;">
        <div class="modal-content bg-dark text-white">
            <div class="modal-header">
                <h2 class="modal-title" id="modalCadastroLabel"><i class="mdi mdi-lightbulb-on"></i>&nbsp;Cadastrar Backlog</h2>
            ...
            <form id="addBacklogForm" action="seu_script.php" method="post" enctype="multipart/form-data" onsubmit="event.preventDefault(); submitAddForm();">
                <div class="modal-body">
                    <!-- Campos para a inserção de dados no backlog -->
                    <div class="mb-3">
                        <!-- Projeto -->
                    ...
                    <div class="mb-3">
                        <!-- Encaminhado para OS -->
                        <label for="addEncaminhadoOs" class="form-label">Enviado para OS</label>
        ...
        <div class="modal-content bg-dark text-white">
            <div class="modal-header">
                <h2 class="modal-title" id="modalVisualizacaoLabel">
                    <i class="mdi mdi-eye-outline"></i>&nbsp;Visualizar Backlog
                </h2>
            ...
            <div class="modal-body">
                <div class="os-details">
                    ...
                    <div class="mb-3">
                        <h3 class="form-label">Enviado para OS</h3>
...
<div id="editModal" class="modal" style="display: none;">
     <div class="modal-dialog modal-dialog-centered" style="max-width: 90%;">
        <div class="modal-content bg-dark text-white">
            <div class="modal-header">
                <h2 class="modal-title" id="modalEdicaoLabel"><i class="mdi mdi-pencil"></i>&nbsp;Editar Backlog</h2>
        ...
        <form id="editBacklogForm" action="process_edit_backlog.php" method="post" enctype="multipart/form-data" onsubmit="event.preventDefault(); submitEditForm();">
            ...
            <!-- Encaminhado para OS -->
                    <div class="mb-3">
                        <label for="editEncaminhadoOs" class="form-label">Enviado para OS</label>
                        <select id="editEncaminhadoOs" name="encaminhado_os" class="form-select bg-secondary text-white" required>
                            <option value="0" <?php echo $backlog['Encaminhado_os'] == "0" ? 'selected' : ''; ?>>Não</option>
                    <option value="1" <?php echo $backlog['Encaminhado_os'] == "1" ? 'selected' : ''; ?>>Sim</option>
                        </select>
                    </div>
...
function loadBacklogDetails(id) {
    ...
    if (backlogItem) {
        ...
        document.getElementById('editEncaminhadoOs').checked = backlogItem.Encaminhado_os === '0';
...
function submitEditForm(event) {
    ...
    .then(response => {
        if (!response.ok) {
            return response.json().then(data => {
                throw new Error(data.message || 'Erro ao atualizar o item do backlog.');
            });
        }
    ...
    .then(data => {
        if(data.success) {
            alert(data.message);
            window.location.href = 'backlog.php'; // Redireciona para backlog.php
        } else {
    ...
    .catch(error => {
        ...
        alert(error.message || 'Erro ao atualizar o item do backlog'); // Exibe o erro em um alerta
    })
    .finally(() => {
...
document.addEventListener('DOMContentLoaded', (event) => {
    document.getElementById('addBacklogForm').addEventListener('submit', function(e) {
...
var backlogs = <?php echo json_encode($backlogs); ?>;
  ...
  if (backlogItem) {
    ...
    var formattedDate = formatDate(backlogItem.Dt_criacao);    
    ...
    if (backlogItem.Descricao && backlogItem.Descricao.trim() !== '') {
      ...
      document.getElementById('viewDescricao').innerHTML = backlogItem.Descricao.replace(/\n/g, '<br>');
    } else {
...
<script> 
function togglePriority(checkbox, backlogId) {
    var row = checkbox.closest('tr');
    if (checkbox.checked) {
        row.classList.add('highlighted');
        localStorage.setItem('highlightedRow' + backlogId, 'true');
    } else {
        row.classList.remove('highlighted');
        localStorage.removeItem('highlightedRow' + backlogId);
    }
    sortHighlightedRows();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    checkboxes.forEach(function(checkbox) {
        var backlogId = checkbox.dataset.backlogId;
        checkbox.addEventListener('change', function() {
            togglePriority(this, backlogId);
        });

        if (localStorage.getItem('highlightedRow' + backlogId) === 'true') {
...
function atualizarContadores() {
    ...
    var ano = urlParams.get('year'); // Verifica se o parâmetro 'year' existe na URL

    // Monta a URL para o endpoint PHP
    var url = 'obter_totais_backlogs.php';
    if (ano) {
        // Se o parâmetro 'year' está presente e não é vazio, adiciona à URL
        url += '?year=' + ano;
    }

    // Faz a requisição para o endpoint PHP e atualiza os contadores
    fetch(url)
        .then(response => response.json())
        .then(data => {
            document.getElementById('totalBacklogs').textContent = data.totalBacklogs;
...
function filterBacklogs(filtro) {
    var urlParams = new URLSearchParams(window.location.search);
    if(filtro) {
