...
.nav-item .btn-success:hover {
  ...
  <div class="input-group searchbar">
    ...
    <input type="text" class="form-control" id="searchInput" placeholder="Pesquisar..." aria-describedby="search">
  </div>
</div>
...
$(document).ready(function() {
    $('#searchInput').on('input', function() {
        ...
        if (searchText.length > 2) {
            var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?busca=' + encodeURIComponent(searchText);
            window.history.pushState({path:newurl},'',newurl);
            searchCards(searchText); // Chamada da função para buscar os cards
        } else {
            // Limpa os resultados de busca ou restaura o estado original da página
            clearSearchResults();
        }
    });
});
...
function searchCards(query) {
    $.ajax({
        url: 'search_cards.php', // A URL do seu script PHP
        type: 'GET',
        data: { search: query }, // Envia o termo de pesquisa
        dataType: 'json',
        success: function(response) {
            // Aqui você atualiza o DOM com os resultados
            console.log(response); // Para fins de debug
            // Exemplo de atualização do DOM
            $('#taskboard').empty();
            response.forEach(function(card) {
                // Adicione cada card ao DOM
                addCardToDOM(card);
            });
        },
        error: function(xhr, status, error) {
...
function clearSearchResults() {
    // Limpa os resultados da busca ou restaura o estado original da página
    // Por exemplo, você pode recarregar os cards ou limpar o que foi exibido
    $('#taskboard').empty();
    // Aqui, você precisa chamar uma função que recarrega os cards ou o estado original da página
    loadCards(false); // false para indicar que não queremos os cards arquivados, por exemplo
}

// Função para adicionar um card ao DOM
function addCardToDOM(card) {
    // Aqui vai sua lógica para criar o HTML de cada card e adicioná-lo ao DOM
    var cardHtml = '<div class="card">' + card.title + '</div>'; // Substitua por sua lógica de criação de card
    $('#taskboard').append(cardHtml);
}

// Função para carregar os cartões inicialmente (você pode ter uma função semelhante já)
function loadCards(isArchived) {
    // Sua lógica para carregar os cards
}
