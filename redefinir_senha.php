<?php
require 'db.php'; // Inclui o script de conexão com o banco de dados
...
if (isset($_GET['token'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM usuarios WHERE Token = ? AND TokenExpiracao > NOW()");
    $stmt->execute([$token]);
    $usuario = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($novaSenha != $confirmarSenha) {
        ...
        $stmt = $pdo->prepare("UPDATE usuarios SET Senha = ?, Token = NULL, TokenExpiracao = NULL WHERE Token = ?");
...
<html dir="ltr">
  ...
  <body>
    <div class="main-wrapper">
      ...
      <!-- -------------------------------------------------------------- -->
      <div class="main-wrapper">
        <div class="preloader"> <!-- Seu SVG de preloader aqui --> </div>
    ...
    <div>
        <div class="logo text-center">
            ...
            <h5 class="font-weight-medium mb-3 mt-1">Redefinir Senha</h5>
        </div>
        <!-- Form -->
        <div class="row">
            <div class="col-12">
                <?php if ($mensagem): ?>
                    ...
                    <input type="hidden" name="token" value="<?php echo htmlspecialchars($token); ?>">
                        <div class="mb-3 row">
                            <div class="col-12">
                                <input class="form-control" type="password" name="novaSenha" required placeholder="Nova senha">
                            </div>
                        </div>
                        <div class="mb-3 row">
                            <div class="col-12">
                                <input class="form-control" type="password" name="confirmarSenha" required placeholder="Confirmar nova senha">
                            </div>
                        </div>
                        <div class="text-center">

                        <div class="col-xs-12">
                            <button class="btn d-block w-100 btn-info" type="submit">Redefinir senha</button>
                        </div>
                    </div>
                </form>
            </div>
